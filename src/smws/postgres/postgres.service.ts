...
export class PostgresService {
  ...
  async populateDistilleries(): Promise<number> {
    ...
    if (countResult > 0) {
      this.logger.log(
        `smws_distilleries already has ${countResult} records, skipping population`,
      );
      return countResult;
    }
    ...
    for (const [category, data] of Object.entries(
      distilleriesData as unknown as Record<
        string,
        {
          distilleries: {
            smwsId: number | string;
    ...
    this.logger.log(
      `Successfully populated ${entries.length} distillery records`,
    );
   ...
   */
  async cleanInvalidEntries(): Promise<{
    deletedLive: number;
    ...
    this.logger.log(
      `Cleaned invalid entries — smws_live: ${deletedLive}, smws_archive: ${deletedArchive}`,
    );
